...
In this tutorial we'll be doing a code-along using the __BigQuery Browser Tool__, which allows you to load data, run queries, and download the results all in your web broswer! If you're interested in using BigQuery without this tool, check out Google's page on [synchronous and asynchronous queries](https://developers.google.com/bigquery/querying-data). 

###Set Up

In order to follow along you'll need to start a sample project on Google BigQuery by going to the [Google API Console](https://console.developers.google.com/project). All you need to do is sign in to your Google Account (or create one), and select `Create Project`. The project name can be anything, but the project ID has to be unique across all of BigQuery (it's like picking a Twitter username all over again!). In a few moments, your project should pop up.

![](http://f.cl.ly/items/1v232B2K3J2I102A2f2t/bigQuery%20-%20project%20being%20created.png)

![](http://f.cl.ly/items/1K38363L3e3E3E1O0E2L/bigQuery%20-%20project%20%20was%20created.png)
...
![](http://f.cl.ly/items/1b2T2a2G2c1i0J2B3r2o/Screen%20Shot%202014-08-29%20at%203.25.09%20PM.png)

![](http://f.cl.ly/items/0V3x1P0F0f3o1R363a0W/Screen%20Shot%202014-08-29%20at%203.24.41%20PM.png)
...
![](http://f.cl.ly/items/2p2q0h0s0132201d0X2U/Screen%20Shot%202014-08-29%20at%203.28.25%20PM.png)

This dataset contains the __words__ in Shakespeare's works, the __word_count__ for the word, in which __corpus__ the word appears, and the __date__ the corpus was written. 
...
Why don't we try and do the same with programming languages over all repositories in GitHub. Here, we're going to make a graph that compares two or more github languages over time. 

![](http://i.imgur.com/xURukGw.png)

####Gather GitHub Data

For this graph, we're going to need access to the GitHub Archive data, which they have conveniently provided for us through the BigQuery browser tool. To get the data, press the little arrow next to your project name, click "Switch to project > Display project..." and type in "githubarchive". A new dataset should appear in the sidebar with three tables. We'll be using the "timeline" table which has data for every repository created since around 2008! Click on the "timeline" dataset to see the table shema and available query fields.
...
Download the template [here](https://github.com/Thinkful/guide-google-bigquery). Save it in the same folder as your data. Then, open terminal, cd into that folder and run "pytnon -m SimpleHTTPServer". In your browser, navigate to `http://localhost:8000/` and your graph should pop up.
...
You can see more detailed quotas and pricing [here](https://developers.google.com/bigquery/pricing), but it should be free for reasonable sized uploads and queries. First off, loading data into BigQuery is __free__! Querying data cost $5 per TB, but the first 1 TB of data processed per month is also __free__. Exporting data is also a __free__ service. So, if you just want to play around, don't worry about accumulating a large receipt (we haven't been charged a penny, and we've been using BigQuery heavily for a few weeks). 

To upload your own data, simply press the little arrow next to your project name. Then choose the file you want to upload, click through the instructions, and you're all set! One of the last steps is creating a schema for the table. Basically, for each column you type out its name and what type of value it contains. So, the Shakespeare dataset we just worked with would have the schema: "word:string, word_count:integer, corpus:string, corpus_date:integer".
